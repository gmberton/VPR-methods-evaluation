...
class TestDataset(data.Dataset):
    def __init__(self, dataset_folder, database_folder="database",
        ...
        if len(self.database_paths) == 0:
            raise FileNotFoundError(f"There are no images in {self.database_folder} , you should change this path")
        if len(self.queries_paths) == 0:
            raise FileNotFoundError(f"There are no images in {self.queries_paths} , you should change this path")
